...
fn sled_vec_test() {
    let v = Vec::default();
    let db = sled_db_open("/tmp/vec_sled_test/04", false).unwrap();
...
fn sled_map_test() {
    let m = Map::default();
    let db = sled_db_open("/tmp/map_sled_test/04", false).unwrap();
...
fn sled_value_test() {
    let v = Value::default();
    let db = sled_db_open("/tmp/value_sled_test/04", false).unwrap();
